  ...
  const setupLenis = useCallback(() => {
    if (useWindowScroll) {
      const lenis = new Lenis({
        ...
        smoothWheel: true, touchMultiplier: 2, infinite: false, wheelMultiplier: 1, lerp: 0.1,
        syncTouch: true, syncTouchLerp: 0.075
      });
      ...
      const lenis = new Lenis({
        ...
        normalizeWheel: true, wheelMultiplier: 1, lerp: 0.1,
        syncTouch: true, syncTouchLerp: 0.075
      });
      lenis.on('scroll', handleScroll);
      const raf = time => { lenis.raf(time); animationFrameRef.current = requestAnimationFrame(raf); };
