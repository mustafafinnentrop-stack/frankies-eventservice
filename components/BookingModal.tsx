...
export default function BookingModal({ onClose }: { onClose: () => void }) {
  ...
  const step1OK = !!(form.eventType && form.guestCount && form.eventDate && form.eventLocation)
  ...
  return (
    <div
      style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(6px)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}
      onClick={e => { if (e.target === e.currentTarget) onClose() }}
    >
      <div style={{ background: 'var(--color-surface)', border: '1px solid rgba(200,164,78,0.22)', maxWidth: '560px', width: '100%', maxHeight: '90vh', overflowY: 'auto', position: 'relative' }}>
        ...
        <div style={{ padding: '2rem' }}>
          ...
          {!done && step === 1 && (
            <div>
              <p style={label}>Art der Veranstaltung *</p>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem', marginBottom: '1.5rem' }}>
                {EVENT_TYPES.map(t => (
                  <button key={t} type="button" onClick={() => set('eventType', t)} style={{ ...chip, ...(form.eventType === t ? chipActive : {}) }}>
                    {t}
                  </button>
                ))}
              </div>

              <p style={label}>Anzahl Gäste *</p>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem', marginBottom: '1.5rem' }}>
                {GUEST_COUNTS.map(g => (
                  <button key={g} type="button" onClick={() => set('guestCount', g)} style={{ ...chip, ...(form.guestCount === g ? chipActive : {}) }}>
                    {g}
                  </button>
                ))}
              </div>

              <p style={label}>Wunschdatum *</p>
              <input type="date" value={form.eventDate} onChange={e => set('eventDate', e.target.value)} style={input} min={new Date().toISOString().split('T')[0]} />

              <p style={label}>Veranstaltungsort *</p>
              <input type="text" placeholder="z.B. Festzelt Kirchhundem" value={form.eventLocation} onChange={e => set('eventLocation', e.target.value)} style={input} />
          ...
          {!done && step === 2 && (
            <div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
                <div>
                  <p style={label}>Vorname *</p>
                  <input type="text" placeholder="Max" value={form.firstName} onChange={e => set('firstName', e.target.value)} style={input} />
                </div>
                <div>
                  <p style={label}>Nachname *</p>
                  <input type="text" placeholder="Muster" value={form.lastName} onChange={e => set('lastName', e.target.value)} style={input} />
                </div>
              </div>

              <p style={label}>E-Mail *</p>
              <input type="email" placeholder="max@beispiel.de" value={form.email} onChange={e => set('email', e.target.value)} style={input} />

              <p style={label}>Telefon *</p>
              <input type="tel" placeholder="+49 151 234 567" value={form.phone} onChange={e => set('phone', e.target.value)} style={input} />

              <p style={label}>Straße &amp; Hausnummer *</p>
              <input type="text" placeholder="Musterstraße 7" value={form.street} onChange={e => set('street', e.target.value)} style={input} />

              <div style={{ display: 'grid', gridTemplateColumns: '5rem 1fr', gap: '0.75rem' }}>
                <div>
                  <p style={label}>PLZ *</p>
                  <input type="text" placeholder="57368" value={form.zip} onChange={e => set('zip', e.target.value)} style={input} maxLength={5} />
                </div>
                <div>
                  <p style={label}>Ort *</p>
                  <input type="text" placeholder="Lennestadt" value={form.city} onChange={e => set('city', e.target.value)} style={input} />
                </div>
              </div>

              <p style={label}>Nachricht (optional)</p>
              <textarea placeholder="Besondere Wünsche, Fragen..." value={form.message} onChange={e => set('message', e.target.value)} rows={3} style={{ ...input, resize: 'vertical' }} />
